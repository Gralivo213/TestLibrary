...
// Check for Asset Interaction (Herb or Tree)
    ...
    drawRegions(timestamp) {
            ...
            const lineDuration = 1000; // Time to draw full perimeter
            const fillDelay = 800;    // When fill starts appearing
            const fillDuration = 500;

            this.regions.forEach(reg => {
                // Get Screen Corners
                // Note: -1 adjustment because internal gx/gy is 0-indexed, but input is 1-indexed
                const p1 = this.gridToScreen(reg.x1 - 1, reg.y1 - 1);
                const p2 = this.gridToScreen(reg.x2, reg.y1 - 1);
                const p3 = this.gridToScreen(reg.x2, reg.y2);
                const p4 = this.gridToScreen(reg.x1 - 1, reg.y2);

                const points = [p1, p2, p3, p4, p1];
                const progress = Math.min(elapsed / lineDuration, 1);

                // 1. Draw Animated Borders
                this.ctx.beginPath();
                ...
                for (let i = 0; i < totalSegments; i++) {
                    const segmentProgress = Math.max(0, Math.min(1, (progress * totalSegments) - i));
                ...
                // 2. Draw Fill Animation
                if (elapsed > fillDelay) {
                    const fillProgress = Math.min((elapsed - fillDelay) / fillDuration, 1);
                    const scale = 1.2 - (0.2 * fillProgress); // Starts 20% bigger
                    const alpha = 0.2 * fillProgress;
                    ...
                    this.ctx.fillStyle = reg.color.replace('#', 'rgba('); // Simple conversion trick
                    // Overriding with a hardcoded rgba for the semi-transparent forest look
