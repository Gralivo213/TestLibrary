...
(function (global) {
    ...
    function getPixelColor(variation, x, y) {
        // Deterministic noise
        const vector = Math.sin(x * 12.9898 + y * 78.233 + variation) * 43758.5453;
        const rand = vector - Math.floor(vector);

        switch (variation) {
            case 0: return (rand > 0.8) ? PALETTE.GRASS_LIGHT : ((rand > 0.4) ? PALETTE.GRASS_MID : PALETTE.GRASS_BASE);
            case 1: return (rand > 0.9) ? PALETTE.GRASS_HIGH : ((rand > 0.6) ? PALETTE.GRASS_LIGHT : ((rand > 0.3) ? PALETTE.GRASS_BASE : PALETTE.GRASS_SHADOW));
            case 2: return (((x + y) % 2 === 0) ? ((rand > 0.5) ? PALETTE.GRASS_MID : PALETTE.GRASS_BASE) : ((rand > 0.5) ? PALETTE.GRASS_LIGHT : PALETTE.GRASS_MID));
            case 3:
                const d = Math.sqrt((x - 8) ** 2 + (y - 8) ** 2);
                if ((rand + d / 10) < 0.6) return PALETTE.DIRT_LIGHT;
                return (rand > 0.8) ? PALETTE.GRASS_LIGHT : PALETTE.GRASS_BASE;
            case 4: return (rand > 0.96) ? PALETTE.FLOWER_P : ((rand > 0.92) ? PALETTE.FLOWER_Y : ((rand > 0.5) ? PALETTE.GRASS_MID : PALETTE.GRASS_BASE));
            case 5: return (rand > 0.7) ? PALETTE.DEAD_GRASS : ((rand > 0.4) ? PALETTE.GRASS_BASE : PALETTE.DIRT_MED);
        }
        return PALETTE.GRASS_BASE;
    }
        ...
        createUI() {
            ...
            style.textContent = `
               ...
               .iso-play-icon {
                ...
                .iso-region-btn.active {
                    border-color: #e3c076;
                    filter: grayscale(0) opacity(1);
                    box-shadow: 0 0 15px rgba(227, 192, 118, 0.4);
                }
                .iso-herb-panel {
                    position: fixed;
                    right: 0;
...
.growth-track { height: 10px; width: 100%; background: linear-gradient(90deg, #d32f2f, #fbc02d, #388e3c); border-radius: 5px; border: 1px solid #3e2723; position: relative; }
        ...
        generateProceduralAssets() {
            ...
            const PIXEL_SCALE = 2; // Results in 64px width (16*2*2) matches CONFIG.TILE_WIDTH
            const WIDTH = CONFIG.TILE_WIDTH;
            const HEIGHT = CONFIG.TILE_HEIGHT + CONFIG.TILE_THICKNESS + 10; // Buffer
            ...
            for (let v = 0; v < 6; v++) {
                ...
                const cx = WIDTH / 2;
                // Align top of diamond (y=0 in grid) to roughly row 0 of canvas
                // The isometric projection pushes Y down by x+y.
                // We shift down slightly so the top pixel isn't cut off.
                const topShift = 0;

                for (let y = 0; y < TEXTURE_RES; y++) {
                    for (let x = 0; x < TEXTURE_RES; x++) {
                        const screenX = (x - y) * PIXEL_SCALE;
                        const screenY = (x + y) * (PIXEL_SCALE / 2);

                        const drawX = cx + screenX - (PIXEL_SCALE / 2);
                        const drawY = topShift + screenY; // + (TEXTURE_RES * PIXEL_SCALE * 0.5);

                        // 1. Dirt Pillar
                        const depth = CONFIG.TILE_THICKNESS / PIXEL_SCALE;
                        ctx.fillStyle = (x === TEXTURE_RES - 1 || y === TEXTURE_RES - 1) ? PALETTE.DIRT_DARK : PALETTE.DIRT_MED;
                        ctx.fillRect(drawX, drawY, PIXEL_SCALE, PIXEL_SCALE + (depth * PIXEL_SCALE));

                        // 2. Grass Top
                        ctx.fillStyle = getPixelColor(v, x, y);
                        ctx.fillRect(drawX, drawY, PIXEL_SCALE, PIXEL_SCALE);
                    }
                }
                this.grassSprites[v] = c;
            }
        }
                ...
                for (let x = 0; x < CONFIG.CHUNK_SIZE; x++) {
                    ...
                    if (!this.chunks[key]) {
                        this.chunks[key] = {
                            ...
                            assetHidden: false,
                            variation: Math.floor(Math.random() * 6) // For procedural grass
                        };
                    }
                }
            }
        ...
        getRegion(tile) {
            return this.regions.find(r =>
                tile.lx >= r.x1 && tile.lx <= r.x2 &&
                tile.ly >= r.y1 && tile.ly <= r.y2
            );
        }
        ...
        drawTile(tile, timestamp) {
            ...
            // Check Region Type
            const region = this.getRegion(tile);
            const isForest = (region && region.type === 'Forest');

            if (isForest && this.grassSprites[tile.variation]) {
                // --- Procedural Draw ---
                const sprite = this.grassSprites[tile.variation];
                this.ctx.imageSmoothingEnabled = false; // Keep pixelated look

                // Draw Sprite
                // The sprite was generated at CONFIG size. We scale by zoom.
                // Alignment: x,y in render loop is the TOP corner of the diamond.
                // Our sprite generation logic placed the top corner at (Width/2, 0).
                const drawW = sprite.width * this.camera.zoom;
                const drawH = sprite.height * this.camera.zoom;

                // Centering: 
                // x is screen X of top corner. Sprite center is (Width/2).
                // So we draw at x - (drawW / 2).
                this.ctx.drawImage(sprite, x - (drawW / 2), y, drawW, drawH);

                // Re-apply path shape for hover/click detection (invisible path)
                this.ctx.beginPath();
                this.ctx.moveTo(x, y);
                ...
                // We still need the stroke/hover logic below to work, but we skip the standard fill
                // if we drew the sprite.
            } else {
                // --- Standard Draw ---
                // 1. Geometry
                this.ctx.beginPath();
            ...
            if (this.state === 'SHOW_RANGE' || this.state === 'SENSE_ACTIVE') {
                ...
                if (move) {
                    // We need to re-define path for fill if we used sprite
                    if (isForest) {
                        this.ctx.beginPath();
                        this.ctx.moveTo(x, y);
                        this.ctx.lineTo(x + w / 2, y + h / 2);
        ...
        drawRegions(timestamp) {
            ...
            if (!this.showRegions || this.regions.length === 0) return;

            const elapsed = timestamp - this.regionAnimStart;
            const fillDuration = 800;

            this.regions.forEach(reg => {
                // Correct Isometric corners for the square fill
                const p1 = this.gridToScreen(reg.x1, reg.y1);
                const p2 = this.gridToScreen(reg.x2 + 1, reg.y1);
                const p3 = this.gridToScreen(reg.x2 + 1, reg.y2 + 1);
                const p4 = this.gridToScreen(reg.x1, reg.y2 + 1);

                // Draw Fill Animation only
                const fillProgress = Math.min(elapsed / fillDuration, 1);
                const scale = 1.1 - (0.1 * fillProgress);
                const alpha = 0.15 * fillProgress; // Slightly more subtle overlay
                ...
                // Purely visual overlay, no hard borders
