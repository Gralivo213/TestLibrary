/**
 * IsoMapEngine Library v2.2 (Animation & UI Update)
 * Features:
 * - Progressive Loading (Assets pop in).
 * - Smooth Movement Animation (Tile by tile).
...
(function(global) {
    ...
    const CONFIG = {
        ...
        ANIMATION: {
            STAGGER_DELAY: 100, 
            FADE_DURATION: 300,
            MOVE_SPEED: 0.15 // Speed of player movement (0.0 to 1.0 per frame approx)
        },
        ASSETS: {
            'M': [
    ...
    class IsoGameEngine {
        constructor() {
            ...
            this.player = {
                chunkId: 1,
                lx: 0, ly: 0,           // Logical Target Position
                prevLx: 0, prevLy: 0,   // Previous Logical Position
                visualLx: 0, visualLy: 0, // Visual Render Position (Float)
                isAnimating: false,
                pathQueue: [],          // Queue of tiles to move through
                moveProgress: 0         // 0.0 to 1.0 for current tile step
            };
        ...
        start() {
            ...
            this.active = true;
            this.createCanvas();
            this.createUI(); // Create UI overlays
            ...
            this.attachEvents();
            this.loop();
            console.log("IsoGameEngine: Started v2.2");
        }
        ...
        createUI() {
            ...
            copyBtn.innerHTML = '📋'; // Simple Clipboard Icon
        ...
        parseMapData(dataStr) {
            ...
            if (newPosMatch) {
                ...
                // Check if we have a previous position defined to animate FROM
                if (prevPosMatch) {
                    ...
                    // Trigger Replay Logic (Teleport to Prev, Animate to New)
                    this.replayAnimation();
                } else {
                    // No history, just teleport
                    this.player.lx = newLx;
                    this.player.ly = newLy;
        ...
        replayAnimation() {
            // 1. Teleport visual to previous
            this.player.visualLx = this.player.prevLx;
            this.player.visualLy = this.player.prevLy;
            
            // 2. Build path from Prev -> Current
            // Note: We use a simplified path builder here just to get the steps
            // This assumes the move was valid.
            ...
            if (startTile && endTile) {
                // If distance is large, just jump, else path
                const path = this.buildPathNodes(startTile, endTile);
                this.player.pathQueue = path; // Array of {lx, ly} to visit
                this.player.isAnimating = true;
                this.player.moveProgress = 0;
            } else {
                // Fallback if tiles missing
        ...
        buildPathNodes(startTile, endTile) {
            const nodes = [];
            // Simple Manhattan path generation for animation
            // 1. Move X
        ...
        resetSelection() {
            ...
            this.validMoves = [];
            this.selectedPath = [];
            // Hide popup on reset? Maybe keep it until new selection.
        }

        calculateManhattanMoves() {
            if (!this.player) return;
            this.validMoves = [];
            const pLx = this.player.lx; // Use logical position for calculation
        ...
        handleTileClick(tile, isRightClick) {
            ...
            if (isPlayerTile) {
                ...
                if (this.state === 'SHOW_RANGE') {
                    ...
                    // Clicked again on target? Maybe confirm? 
                    // For now, just keep showing popup
                }
            }
        }
        ...
        drawPlayer() {
            ...
            // We calculate screen pos based on VISUAL coordinates (float)
            // Need to convert visualLx/Ly to global grid coords. 
            // The engine assumes a single chunk setup for now based on generation code,
            // but for safety we map visual Lx/Ly to the same Gx/Gy logic.
            // GX = (ChunkX * Size) + Lx - 1 (since lx is 1-based in storage but 0-based in grid logic?)
            // Actually generateChunk uses: gx = (chunkX * 10) + x. lx = x+1.
            // So gx = lx - 1 (if chunkX is 0).
            
            // Assuming chunk 0,0 for visual calc (or current player chunk offset)
            const gx = this.player.visualLx - 1; // Convert 1-based to 0-based grid
            ...
            // Token Side
            this.ctx.save();
            this.ctx.translate(x, y + (CONFIG.TILE_HEIGHT * this.camera.zoom)/2); // Center on tile top
            this.ctx.scale(1, 0.5); 
            this.ctx.beginPath();
        ...
        updateAnimation() {
            ...
            // Simple Linear Interpolation between visualPos and targetNode
            if (this.player.pathQueue.length > 0) {
                const target = this.player.pathQueue[0];
                const dx = target.lx - this.player.visualLx;
                const dy = target.ly - this.player.visualLy;
                const dist = Math.sqrt(dx*dx + dy*dy);

                if (dist < CONFIG.ANIMATION.MOVE_SPEED) {
                    // Arrived at this node
                    this.player.visualLx = target.lx;
                    this.player.visualLy = target.ly;
                    this.player.pathQueue.shift(); // Remove node
                } else {
                    // Move towards
                    this.player.visualLx += (dx / dist) * CONFIG.ANIMATION.MOVE_SPEED;
                    this.player.visualLy += (dy / dist) * CONFIG.ANIMATION.MOVE_SPEED;
                }
            } else {
                this.player.isAnimating = false;
                // Snap to final to be sure
                this.player.visualLx = this.player.lx;
                this.player.visualLy = this.player.ly;
            }
        }
        ...
        render(timestamp) {
            ...
            // Draw Player Indepedently on top
            this.drawPlayer();
        }
