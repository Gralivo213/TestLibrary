/**
 * IsoMapEngine Library v1.4
 * Updates: 
 * - Added Player entity rendering
 * - Added Herb (H) assets and parsing
 * - Improved parser robustness (handles messy braces and dot/comma separators)
 */
...
(function(global) {
    ...
    class IsoGameEngine {
        ...
        parseMapData(dataStr) {
            ...
            // 2. Player Position
            // Matches "Player = 3,3" or "Player = 3.3"
            ...
            // This ignores structure/braces and finds "Key : coords" anywhere
            ...
            while ((match = lineRegex.exec(dataStr)) !== null) {
                ...
                coordPairs.forEach(pair => {
                    ...
                    // Handle comma or dot separator (3,1 or 3.1)
                    ...
                    if (parts.length >= 2) {
                        ...
                        this.updateTileAsset(chunkId, lx, ly, assets);
                    }
                });
            }
        }
        ...
        drawTile(tile) {
            ...
            // --- Draw Environmental Asset (Mountains/Herbs) ---
            if (tile.assetUrl) {
            ...
            // --- Draw Player ---
            // Check if this tile is the player's current location
            if (this.player && this.player.chunkId === tile.chunkId && 
                ...
                if (pImg.complete && pImg.naturalWidth > 0) {
                    // Player scaling (slightly smaller than full tile width usually looks better)
                    const pScale = (w / pImg.naturalWidth) * 0.6;
                    const pw = pImg.naturalWidth * pScale;
                    const ph = pImg.naturalHeight * pScale;
                    
                    const px = x - (pw / 2);
                    // Center player on the tile (feet at center)
                    const py = (y + h/2) - ph + (h * 0.5);
                    
                    this.ctx.drawImage(pImg, px, py, pw, ph);
                }
            }
        }
