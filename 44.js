/**
 * IsoMapEngine Library v1.6
 * Updates: 
 * - Added 'LastPosition' and 'NewPosition' parsing
 * - Added Movement Range Logic (Dist <= 2, Blue Glow)
 * - Added Distance-based Gradient (Darker near, Lighter far)
 * - Added Interaction: Left Click to Highlight/Select, Right Click to Deselect
...
(function(global) {
    ...
    class IsoGameEngine {
        ...
        parseMapData(dataStr) {
            ...
            // Supports: 'Player', 'NewPosition', 'LastPosition' (LastPosition is parsed but visual focus is on New)
            
            // Regex for Current/New Position
            const newPosMatch = dataStr.match(/(?:Player|NewPosition)\s*=\s*(\d+)[.,](\d+)/i);
            // Regex for Previous Position (Fallback if New isn't defined, or just for state)
            ...
            if (newPosMatch) {
                ...
                // If only previous is given, render there
        ...
        calculateMovementRange() {
            ...
            // Iterate all tiles in current chunk (Optimization: only check nearby)
            Object.values(this.chunks).forEach(tile => {
                if (tile.chunkId !== this.player.chunkId) return;

                // Chebyshev Distance (King's Move)
                const dx = Math.abs(tile.lx - pLx);
                const dy = Math.abs(tile.lx - pLx); // wait, bug in original thought? No, lx/ly
                const dist = Math.max(Math.abs(tile.lx - pLx), Math.abs(tile.ly - pLy));

                // Range limit: 2. Exclude self (dist 0).
                if (dist > 0 && dist <= 2) {
        ...
        handleTileClick(tile, isRightClick) {
            ...
            if (isPlayerTile) {
                ...
                if (this.highlightedTiles.length > 0) {
                    this.highlightedTiles = []; // Toggle off
                } else {
                    this.calculateMovementRange(); // Toggle on
                }
            } else {
                // Is this a highlighted tile?
                ...
                if (highlightInfo) {
                    ...
                    // Clicked elsewhere, clear selection? (Optional, per requirement mostly keeps quiet)
                    this.selectedTile = null;
                    this.highlightedTiles = [];
                }
            }
        ...
        drawTile(tile) {
            ...
            if (highlight) {
                // Distance based opacity: Dist 1 = 0.5 (Darker), Dist 2 = 0.25 (Lighter)
            ...
            if (this.selectedTile && this.selectedTile === tile) {
                this.ctx.save();
                this.ctx.translate(x, y + h/2); // Center of top face
                this.ctx.scale(1, 0.5); // Isometric squash
                this.ctx.beginPath();
                this.ctx.arc(0, 0, w * 0.3, 0, Math.PI * 2); // Radius 30% of tile width
            ...
            if (tile.assetUrl) {
                ...
                if (img.complete && img.naturalWidth > 0) {
                    let scaleFactor = 0.8;
                    if (tile.assetType === 'H') scaleFactor = 0.5;
            ...
            if (this.player && this.player.chunkId === tile.chunkId && 
                ...
                if (pImg.complete && pImg.naturalWidth > 0) {
                    ...
                    const pScale = (w / pImg.naturalWidth) * 0.7; 
