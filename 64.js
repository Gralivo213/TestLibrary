...
(function (global) {
    ...
    class IsoGameEngine {
        constructor() {
            ...
            this.stamina = 88;
            this.baseStamina = 88; // Added to track stamina before current selection
        ...
        handleTileClick(tile, isRightClick) {
            ...
            if (this.state === 'SHOW_RANGE') {
                ...
                if (move) {
                    ...
                    let currentImpedance = 1;
                    this.regions.forEach(reg => {
                        if (tile.lx >= reg.x1 && tile.lx <= reg.x2 &&
                            tile.ly >= reg.y1 && tile.ly <= reg.y2) {
                            currentImpedance = Math.max(currentImpedance, reg.impedance);
                        }
                    });

                    const cost = move.dist * 10 * currentImpedance;
