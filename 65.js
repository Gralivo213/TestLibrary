...
(function (global) {
    ...
    class IsoGameEngine {
        constructor() {
            ...
            this.stamina = 88;
            this.baseStamina = 88; // Track stamina before current selection
...
<div class="iso-ui-text-layer">
        ...
        handleTileClick(tile, isRightClick) {
            ...
            if (this.state === 'SHOW_RANGE') {
                ...
                if (move) {
                    // Calculate Impedance-based cost
                    let totalImpedance = 1;
                    this.regions.forEach(reg => {
                        if (tile.lx >= reg.x1 && tile.lx <= reg.x2 &&
                            tile.ly >= reg.y1 && tile.ly <= reg.y2) {
                            totalImpedance = Math.max(totalImpedance, reg.impedance);
                        }
                    });

                    const cost = move.dist * 10 * totalImpedance;
                    ...
                    if (this.baseStamina >= cost) {
                        ...
                        console.log("Too exhausted for this move!");
                        this.resetSelection();
                    }
                } else {
                    this.resetSelection();
