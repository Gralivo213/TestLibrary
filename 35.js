...
/* --- STAT POPUP & ANIMATION --- */
                ...
                .iso-stat-popup {
                    ...
                    background-size: 100% 100%; z-index: 4000; opacity: 0; visibility: hidden; pointer-events: none;
                    transition: opacity 0.5s ease, transform 0.5s cubic-bezier(0.22, 1, 0.36, 1), visibility 0.5s;
                    filter: drop-shadow(0 0 20px rgba(0,0,0,0.8));
                }
                .iso-stat-popup.visible { opacity: 1; visibility: visible; pointer-events: auto; transform: translate(-50%, -50%); }
                .iso-stat-close { position: absolute; top: 5%; right: 4%; color: #fbbf24; font-size: 70px; font-weight: bold; cursor: pointer; transition: transform 0.1s; text-shadow: 3px 3px #000; }
                .iso-stat-close:hover { transform: scale(1.2); color: #fff; }

                /* --- JADE STAT SLAB --- */
                .iso-stat-header { position: absolute; top: 12%; left: 10%; right: 10%; display: flex; justify-content: space-between; }
                .iso-stat-section { flex: 1; text-align: center; color: #fbbf24; font-family: 'KoreanCalligraphy', serif; font-size: 2.5vw; border-bottom: 1px solid rgba(251, 191, 36, 0.5); }
                .iso-stat-section.active { color: #fff; border-bottom: 2px solid #fbbf24; text-shadow: 0 0 10px rgba(251, 191, 36, 0.8); }
                
                .iso-stat-body { position: absolute; top: 16%; left: 10%; right: 10%; bottom: 10%; font-family: 'SpellOfAsia', serif; color: #fff; display: flex; flex-direction: column; text-shadow: 2px 2px 4px rgba(0,0,0,0.8); }
                
                .iso-jade-slab {
                    position: absolute; top: 5vw; left: 11vw; width: 60vw;
                    /* Ultra-fine Microscopic Dots & High Transparency */
                    background: 
                        url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.04'/%3E%3C/svg%3E"),
                        linear-gradient(135deg, rgba(45, 77, 48, 0.15) 0%, rgba(26, 48, 28, 0.15) 50%, rgba(13, 26, 14, 0.15) 100%);
                    border: 1px solid rgba(20, 38, 22, 0.3); border-top: 2px solid rgba(124, 179, 128, 0.4); border-left: 2px solid rgba(90, 133, 93, 0.4);
                    border-radius: 7.9vw 4px 4px 7.9vw; /* Curve starts from yin yang radius */
                    padding: 1vw 2vw 1vw 8vw; box-shadow: 0 10px 20px rgba(0,0,0,0.4), inset 0 0 15px rgba(0,0,0,0.3);
                    backdrop-filter: blur(2px);
                }

                .iso-jade-emblem { position: absolute; left: -11.9vw; top: 48%; transform: translateY(-50%); width: 15.8vw; height: 15.8vw; z-index: 5; filter: drop-shadow(0 4px 10px rgba(0,0,0,0.9)); }
                .iso-jade-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.3vw 2vw; }
                .iso-jade-divider { grid-column: span 2; height: 1px; background: linear-gradient(to right, transparent, rgba(229, 192, 107, 0.3), transparent); margin: 0.4vw 0; }
                .iso-jade-line { font-size: 1.4vw; line-height: 1.4; }
                .iso-jade-line .jade-label { color: #e5c06b; font-weight: bold; margin-right: 0.5vw; }
                
                .iso-stat-bio { margin-top: 15vw; margin-left: 0.5vw; font-size: 1.5vw; color: #e5e5e5; }
                .iso-stat-buttons { position: absolute; bottom: 2vw; right: 1vw; display: flex; gap: 1.5vw; }
                .iso-scroll-btn { position: relative; width: 17vw; height: 3.5vw; cursor: pointer; display: flex; align-items: center; justify-content: center; }
                .iso-scroll-btn svg { position: absolute; inset: 0; width: 100%; height: 100%; z-index: -1; filter: drop-shadow(0 6px 8px rgba(0,0,0,0.7)); }
                .iso-scroll-btn-text { color: #2b1d00; font-size: 1.6vw; font-weight: bold; text-shadow: 1px 1px 0px rgba(255, 255, 255, 0.4); }
          ...
          // 3. Build Composite UI (Octagon + Stacked Bars Column)
            ...
            const scrollSvg = `<svg viewBox="0 0 200 60" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="pG" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#f5deb3"/><stop offset="100%" stop-color="#deb887"/></linearGradient><linearGradient id="rG" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="#daa520"/><stop offset="100%" stop-color="#b8860b"/></linearGradient></defs><rect x="15" y="4" width="170" height="52" fill="url(#pG)" stroke="#8b6508" stroke-width="1.5"/><rect x="0" y="0" width="22" height="60" rx="4" fill="url(#rG)" stroke="#5c4000" stroke-width="2"/><rect x="178" y="0" width="22" height="60" rx="4" fill="url(#rG)" stroke="#5c4000" stroke-width="2"/></svg>`;

            const jadeEmblemSvg = `<svg viewBox="0 0 100 100" width="100%" height="100%"><defs><radialGradient id="jG" cx="30%" cy="30%" r="70%"><stop offset="0%" stop-color="#b2d8b2"/><stop offset="100%" stop-color="#2a4d2c"/></radialGradient><linearGradient id="gG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#fceda1"/><stop offset="100%" stop-color="#805b14"/></linearGradient></defs><circle cx="50" cy="50" r="48" fill="url(#jG)"/><circle cx="50" cy="50" r="34" fill="#e8f4e9" stroke="url(#gG)" stroke-width="2.5"/><path d="M 50 16 A 17 17 0 0 1 50 50 A 17 17 0 0 0 50 84 A 34 34 0 0 1 50 16 Z" fill="#254627"/><circle cx="50" cy="33" r="5" fill="#254627"/><circle cx="50" cy="67" r="5" fill="#e8f4e9"/></svg>`;
            ...
            statPopup.innerHTML = `
                ...
                <div class="iso-stat-body" id="stat-info-body">
                    ...
                    <div class="iso-stat-bio">Biography:<br>(Your character history goes here...)</div>
                    <div class="iso-stat-buttons">
                        <div class="iso-scroll-btn">${scrollSvg}<span class="iso-scroll-btn-text">Relation</span></div>
                        <div class="iso-scroll-btn">${scrollSvg}<span class="iso-scroll-btn-text">Karma</span></div>
                    </div>
                </div>
            `;
