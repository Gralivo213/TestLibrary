...
.growth-track { height: 10px; width: 100%; background: linear-gradient(90deg, #d32f2f, #fbc02d, #388e3c); border-radius: 5px; border: 1px solid #3e2723; position: relative; }
        ...
        drawRegions(timestamp) {
            ...
            this.regions.forEach(reg => {
                // Calculate the 4 outer corners of the entire region block
                // Subtract 1 from coordinates because gridToScreen is 0-indexed internally
                const pTop = this.gridToScreen(reg.x1 - 1, reg.y1 - 1);        // Top Corner
                const pRight = this.gridToScreen(reg.x2, reg.y1 - 1);        // Right Corner
                const pBottom = this.gridToScreen(reg.x2, reg.y2);           // Bottom Corner
                const pLeft = this.gridToScreen(reg.x1 - 1, reg.y2);         // Left Corner

                const fillProgress = Math.min(elapsed / fillDuration, 1);
                const alpha = 0.4 * fillProgress;

                this.ctx.save();
                this.ctx.beginPath();
                this.ctx.moveTo(pTop.x, pTop.y);
                this.ctx.lineTo(pRight.x + (CONFIG.TILE_WIDTH / 2 * this.camera.zoom), pRight.y + (CONFIG.TILE_HEIGHT / 2 * this.camera.zoom));
                this.ctx.lineTo(pBottom.x, pBottom.y + (CONFIG.TILE_HEIGHT * this.camera.zoom));
                this.ctx.lineTo(pLeft.x - (CONFIG.TILE_WIDTH / 2 * this.camera.zoom), pLeft.y + (CONFIG.TILE_HEIGHT / 2 * this.camera.zoom));
                this.ctx.closePath();

                // Draw the 'Covering' effect
