...
(function (global) {
    ...
    class IsoGameEngine {
        constructor() {
            ...
            this.stamina = 88; // Default starting stamina
...
<div class="iso-ui-text-layer">
            ...
            const updateIcon = () => {
                const url = `https://raw.githubusercontent.com/Gralivo213/TestLibrary/main/imh/${modes[modeIdx]}`;
            ...
            updateIcon(); // Call once on start

            btn.onclick = (e) => {
                e.stopPropagation();
                modeIdx = (modeIdx + 1) % modes.length;
                updateIcon();
                // New: Reset selection and stamina UI when mode changes to avoid bugs
                this.resetSelection();
        ...
        handleTileClick(tile, isRightClick) {
            ...
            if (this.state === 'SHOW_RANGE') {
                ...
                if (move) {
                    // Reduce stamina based on distance (Distance * 10 cost per tile)
                    const cost = move.dist * 10;
                    this.stamina -= cost;
                    this.updateStaminaUI(this.stamina);
        ...
        calculateManhattanMoves() {
            ...
            // Mode Logic: Walk=2, Run=3, Sneak/Hide=1
            const modes = ['WalkUI.png', 'RunUI.png', 'SneakUI.png', 'HideUI.png'];
            const currentMode = modes[window.modeIdx || 0];
            let range = 2;
