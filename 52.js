...
.iso-herb-panel.visible { opacity: 1; transform: translateY(0); pointer-events: auto; }
              ...
              .iso-msg-box.visible { opacity: 1; transform: translateX(0); }
                ...
                .iso-control-group { display: flex; flex-direction: column; }
                .iso-control-group label { margin-bottom: 2px; color: #fbbf24; }
                .iso-control-group input { cursor: pointer; }
            `;
            ...
            staminaBar.innerHTML = `
                <div class="iso-nameplate" id="iso-player-name">Player</div>
                ...
                <div class="iso-ui-bar-wrapper">                    <div class="iso-ui-bar-bg iso-torn-shape"></div>
...
<div class="iso-ui-text-layer">
            ...
            // --- TEMP NAMEPLATE SLIDERS ---
            const controls = document.createElement('div');
            controls.className = 'iso-temp-controls';
            controls.innerHTML = `
                <div class="iso-control-group">
                    <label>Left (X): <span id="val-x">55</span>px</label>
                    <input type="range" id="slide-x" min="0" max="200" value="55">
                </div>
                <div class="iso-control-group">
                    <label>Top (Y): <span id="val-y">-22</span>px</label>
                    <input type="range" id="slide-y" min="-100" max="100" value="-22">
                </div>
                <div class="iso-control-group">
                    <label>Width: <span id="val-w">100</span>px</label>
                    <input type="range" id="slide-w" min="50" max="300" value="100">
                </div>
                <div class="iso-control-group">
                    <label>Height: <span id="val-h">24</span>px</label>
                    <input type="range" id="slide-h" min="10" max="100" value="24">
                </div>
            `;
            document.body.appendChild(controls);

            const np = document.getElementById('iso-player-name');
            const updateNP = () => {
                const x = document.getElementById('slide-x').value;
                const y = document.getElementById('slide-y').value;
                const w = document.getElementById('slide-w').value;
                const h = document.getElementById('slide-h').value;
                ...
                if (np) {
                    np.style.left = x + 'px';
                    np.style.top = y + 'px';
                    np.style.minWidth = w + 'px';
                    np.style.height = h + 'px';
                }
            };

            ['slide-x', 'slide-y', 'slide-w', 'slide-h'].forEach(id => {
                document.getElementById(id).addEventListener('input', updateNP);
            });
        }
        ...
        parseMapData(dataStr) {
            ...
            if (nameMatch) {
                ...
                if (el) el.textContent = nameMatch[1].trim();
            }
