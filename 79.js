...
.growth-track { height: 10px; width: 100%; background: linear-gradient(90deg, #d32f2f, #fbc02d, #388e3c); border-radius: 5px; border: 1px solid #3e2723; position: relative; }
        ...
        drawRegions(timestamp) {
            ...
            this.regions.forEach(reg => {
                // Precise corner mapping using the grid points directly
                // Top point of tile (x1,y1)
                const pTop = this.gridToScreen(reg.x1 - 1, reg.y1 - 1);
                // Right point of tile (x2,y1)
                const pRight = this.gridToScreen(reg.x2, reg.y1 - 1);
                // Bottom point of tile (x2,y2)
                const pBottom = this.gridToScreen(reg.x2, reg.y2);
                // Left point of tile (x1,y2)
                const pLeft = this.gridToScreen(reg.x1 - 1, reg.y2);
