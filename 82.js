...
drawRegions(timestamp) {
            ...
            const fillDelay = 1200;    
            const fillDuration = 800;

            this.regions.forEach(reg => {
                // Correct Isometric Mapping: Using actual tile corners
                // x1,y1 to x2,y2 covers the full square range.
                const p1 = this.gridToScreen(reg.x1, reg.y1);             // Top-most point
                const p2 = this.gridToScreen(reg.x2 + 1, reg.y1);         // Right-most point
                const p3 = this.gridToScreen(reg.x2 + 1, reg.y2 + 1);     // Bottom-most point
                const p4 = this.gridToScreen(reg.x1, reg.y2 + 1);         // Left-most point

                const points = [p1, p2, p3, p4, p1];
                const progress = Math.min(elapsed / lineDuration, 1);

                // 1. Draw Animated Primary Border (Overlay)
                this.ctx.beginPath();
                this.ctx.strokeStyle = '#4CAF50'; // Darker Forest Green
                ...
                if (elapsed > fillDelay) {
                    ...
                    const scale = 1.3 - (0.3 * fillProgress); // Shrinks from 130% to 100%
                    const alpha = 0.25 * fillProgress;
