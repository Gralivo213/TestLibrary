...
.iso-herb-panel.visible { opacity: 1; transform: translateY(0); pointer-events: auto; }
              ...
              .iso-msg-box.visible { opacity: 1; transform: translateX(0); }
                ...
                .iso-temp-slider-box {
                    ...
                    z-index: 5000; background: rgba(0,0,0,0.9); padding: 15px; border: 1px solid #fbbf24;
                    border-radius: 8px; color: #fff; display: flex; flex-direction: column; align-items: center; gap: 5px;
                }
                .iso-temp-slider-box input { width: 300px; cursor: pointer; }
            `;
          ...
          // 3. Build Composite UI (Octagon + Stacked Bars Column)
            ...
            // --- TEMPORARY SLIDER LOGIC ---
            const sliderBox = document.createElement('div');
            sliderBox.className = 'iso-temp-slider-box';
            sliderBox.innerHTML = `
                <label style="font-family: monospace; font-size: 14px;">Header Y-Pos: <span id="y-val-display">18</span>%</label>
                <input type="range" id="y-slider" min="0" max="100" value="18">
            `;
            document.body.appendChild(sliderBox);

            const header = statPopup.querySelector('.iso-stat-header');
            const slider = document.getElementById('y-slider');
            const display = document.getElementById('y-val-display');
