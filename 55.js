...
.iso-herb-panel.visible { opacity: 1; transform: translateY(0); pointer-events: auto; }
                ...
                .iso-nameplate {
                    position: absolute;
                    top: -22px; /* Position above the bar */
                    left: 50px; /* Connect to the Octagon/Bar */
                    height: 26px;
                    background: #0f0f0f;
                    border-top: 2px solid #fbbf24; /* Gold trim */
                    color: #fbbf24;
                    font-weight: bold;
                    font-size: 12px;
                    display: flex;
                    align-items: center;
                    padding-left: 20px;
                    padding-right: 35px; /* Extra space for the slant */
                    min-width: 120px;
                    white-space: nowrap;
                    z-index: 15; /* Sit behind the octagon but visually connected */
                    clip-path: polygon(0 0, 85% 0, 100% 100%, 0% 100%); /* The Slanted End */
...
<div class="iso-ui-text-layer">
        ...
        parseMapData(dataStr) {
            ...
            // Parse Player Nameplate
            const nameMatch = dataStr.match(/<Player>\s*Name\s*:\s*(.*)\s*<\/Player>/i);
            if (nameMatch) {
                const nameEl = document.getElementById('iso-player-name');
                if (nameEl) nameEl.textContent = nameMatch[1].trim();
            }
        ...
        drawRegions(timestamp) {
            ...
            this.regions.forEach(reg => {
                ...
                this.ctx.strokeStyle = '#4CAF50'; // Solid Green (No transparency fade)
                this.ctx.lineWidth = 5 * this.camera.zoom; // Thicker line
                this.ctx.lineJoin = "round";
                this.ctx.setLineDash([15, 10]); // Dashed effect for better visibility
                this.ctx.stroke();
                this.ctx.setLineDash([]);

                // Apply a very subtle ground overlay
                this.ctx.fillStyle = `rgba(76, 175, 80, 0.15)`; // Constant subtle fill
                this.ctx.fill();
                this.ctx.restore();
            });
        }
        render(timestamp) {
            ...
            // 2. Draw Region Overlays (On ground, below entities)
            this.drawRegions(timestamp);

            // 3. Draw Entities (NPC Pieces followed by Player)
