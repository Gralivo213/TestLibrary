...
(function (global) {
    ...
    const CONFIG = {
        ...
        COLORS: {
            ...
            HIGHLIGHT_RED: 'rgba(220, 38, 38, 0.7)', // Added for blocked tiles
    ...
    class IsoGameEngine {
        ...
        createUI() {
            ...
            // Load Clipboard.js for compatibility
            const clipScript = document.createElement('script');
            clipScript.src = "https://cdnjs.cloudflare.com/ajax/libs/clipboard.js/2.0.11/clipboard.min.js";
            ...
            style.textContent = `
                ...
                /* Bar - Width decreased for a tighter fit */
                .iso-ui-bar-wrapper {
               ...
               .iso-ui-bar-progress {
                ...
                /* Bar Color Logic */
                .iso-ui-bar-progress.stamina { background: linear-gradient(to right, #1e3a8a, #3b82f6, #22d3ee); box-shadow: 0 0 15px #3b82f6; }
                .iso-ui-bar-progress.qi { background: linear-gradient(to right, #854d0e, #eab308, #fef08a); box-shadow: 0 0 15px #eab308; }

                .iso-ui-text-layer {
                    position: relative; z-index: 5; width: 100%; display: flex; justify-content: space-between; align-items: center;
                }

                /* Column stack for the torn-shape bars */
                .iso-ui-bars-column { display: flex; flex-direction: column; gap: 4px; margin-left: -20px; }
...
.iso-herb-panel.visible { opacity: 1; transform: translateY(0); pointer-events: auto; }
              ...
              .iso-msg-box.visible { opacity: 1; transform: translateX(0); }
                ...
                /* --- HEALTH BAR (Stacked in column with premium shading) --- */
                .iso-nameplate {
                    position: relative;
                    height: 20px;
                    width: 190px;
                    background: linear-gradient(to right, #450a0a, #7f1d1d); /* Red shading for lost health */
                ...
                .iso-health-fill {
                    position: absolute; left: 0; top: 0; bottom: 0; width: 100%;
                    background: linear-gradient(to right, #14532d, #16a34a, #4ade80); /* Green shading */
                ...
                .iso-stat-popup {
                    ...
                    /* Initial state: Lower position (-42%) and invisible */
                    transform: translate(-50%, -42%); 
                    width: 90vw; height: 54vw; 
                    background: url('https://raw.githubusercontent.com/Gralivo213/TestLibrary/main/imh/StatUI.png') no-repeat center center;
                    background-size: 100% 100%;
                    z-index: 4000; opacity: 0;
                    visibility: hidden; pointer-events: none;
                    /* Smooth rise and fade transition */
                    transition: opacity 0.5s ease, transform 0.5s cubic-bezier(0.22, 1, 0.36, 1), visibility 0.5s;
                    filter: drop-shadow(0 0 20px rgba(0,0,0,0.8));
                }
                .iso-stat-popup.visible { 
                    opacity: 1; 
                    visibility: visible; 
                    pointer-events: auto;
                    /* Final state: Rises to center (-50%) */
                    transform: translate(-50%, -50%); 
                }
                .iso-stat-close {
                    position: absolute; top: 5%; right: 4%;
                    color: #fbbf24; font-size: 70px; font-weight: bold; /* Scaled 2.5x */
                    cursor: pointer; transition: transform 0.1s;
                    text-shadow: 3px 3px #000;
                }
                .iso-stat-close:hover { transform: scale(1.2); color: #fff; }
                .iso-nameplate { cursor: pointer; }
                ...
                .iso-stat-section {
                    ...
                    color: #fbbf24; font-family: 'KoreanCalligraphy', serif;
                    font-size: 2.5vw; padding-bottom: 5px;
                    /* Height 1 line that spans the width but stays inside margins */
                    border-bottom: 1px solid rgba(251, 191, 36, 0.5);
                    transition: all 0.2s;
                }
                .iso-stat-section.active {
                    color: #fff;
                ...
                /* --- STAT INFO CONTENT --- */
                @font-face {
                    font-family: 'SpellOfAsia';
                    src: url('https://raw.githubusercontent.com/Gralivo213/TestLibrary/main/Font/Spell_of_Asia.ttf');
                }
                
                .iso-stat-body {
                    position: absolute; top: 16%; left: 10%; right: 10%; bottom: 10%;
                    font-family: 'SpellOfAsia', 'KoreanCalligraphy', serif;
                    color: #fff; display: flex; flex-direction: column;
                    text-shadow: 2px 2px 4px rgba(0,0,0,0.8); text-align: left;
                }

                /* --- ENHANCED UNIFIED JADE SLAB --- */
                .iso-jade-slab {
                    position: absolute; /* Changed to absolute for precise coordinate editing */
                    top: 1vw; left: 4vw; width: 60vw; height: auto;
                    /* Semi-transparent deep jade base with noise texture */
                    background: 
                        linear-gradient(rgba(255,255,255,0.05), rgba(0,0,0,0.2)),
                        url("https://www.transparenttextures.com/patterns/carbon-fibre.png"), 
                        linear-gradient(135deg, rgba(45, 77, 48, 0.7) 0%, rgba(26, 48, 28, 0.7) 50%, rgba(13, 26, 14, 0.7) 100%);
                    background-blend-mode: overlay, multiply, normal;
                    /* Hard Beveled Borders */
                    border: 1px solid #142616;
                    border-top: 2px solid #7cb380;
                    border-left: 2px solid #5a855d;
                    border-radius: 4px;
                ...
                /* --- TEMPORARY MULTI-EDITOR UI --- */
                .iso-editor-panel {
                    position: fixed; bottom: 10px; left: 10px; width: 260px; max-height: 80vh;
                    overflow-y: auto; background: rgba(0,0,0,0.9); border: 1px solid #fbbf24;
                    padding: 10px; z-index: 9999; color: #fff; font-family: monospace; font-size: 11px;
                }
                .editor-group { margin-bottom: 10px; border-bottom: 1px solid #333; padding-bottom: 5px; }
                .editor-row { display: flex; flex-direction: column; margin-bottom: 5px; }
                .editor-row label { display: flex; justify-content: space-between; }
                .editor-row input { width: 100%; height: 12px; cursor: pointer; accent-color: #fbbf24; }

                /* Gloss highlight overlay */
                .iso-jade-slab::before {
                ...
                .iso-jade-emblem {
                    position: absolute; left: -3.8vw; top: 50%; transform: translateY(-50%);
                    width: 7.5vw; height: 7.5vw; z-index: 5;
                    filter: drop-shadow(0 4px 8px rgba(0,0,0,0.8));
                }

                .iso-jade-grid {
                    display: grid;
                    grid-template-columns: 1fr 1fr; /* Two columns for layout efficiency */
                    gap: 0.4vw 2vw;
                }
                ...
                .iso-jade-divider {
                    grid-column: span 2;
                    height: 1px;
                    background: linear-gradient(to right, transparent, rgba(229, 192, 107, 0.3), transparent);
                    margin: 0.5vw 0;
                }

                .iso-jade-line { font-size: 1.4vw; line-height: 1.4; text-shadow: 2px 2px 2px #000; }
                .iso-jade-line .jade-label { color: #e5c06b; font-weight: bold; margin-right: 0.5vw; }
                .iso-jade-line .jade-val { color: #fff; }

                .iso-stat-bio { margin-top: 0.5vw; margin-left: 0.5vw; font-size: 1.5vw; color: #e5e5e5; line-height: 1.4; }

                .iso-stat-buttons {
                    position: absolute; bottom: 0; right: 0;
                    display: flex; gap: 1.5vw; pointer-events: auto;
                }

                .iso-scroll-btn {
                    position: relative; width: 11vw; height: 3.5vw; cursor: pointer;
                    display: flex; align-items: center; justify-content: center;
                    transition: transform 0.2s, filter 0.2s;
                }
                .iso-scroll-btn:hover { transform: scale(1.05); filter: brightness(1.15); }
                .iso-scroll-btn svg { 
                    position: absolute; inset: 0; width: 100%; height: 100%; 
                    z-index: -1; filter: drop-shadow(0 6px 8px rgba(0,0,0,0.7)); 
                }
                .iso-scroll-btn-text {
                    color: #2b1d00; font-family: 'SpellOfAsia', serif;
          ...
          // 3. Build Composite UI (Octagon + Stacked Bars Column)
            const statPopup = document.createElement('div');
            statPopup.className = 'iso-stat-popup';
            
            const scrollSvg = `
                <svg viewBox="0 0 200 60" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
                    <defs>
                        <linearGradient id="paperGrad" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stop-color="#f5deb3"/><stop offset="50%" stop-color="#ffebcd"/><stop offset="100%" stop-color="#deb887"/>
                        </linearGradient>
                        <linearGradient id="rollGrad" x1="0" y1="0" x2="1" y2="0">
                            <stop offset="0%" stop-color="#daa520"/><stop offset="40%" stop-color="#ffea70"/><stop offset="100%" stop-color="#b8860b"/>
                        </linearGradient>
                        <filter id="innerShadow">
                            <feDropShadow dx="2" dy="3" stdDeviation="3" flood-color="#000" flood-opacity="0.4"/>
                        </filter>
                    </defs>
                    <rect x="15" y="4" width="170" height="52" fill="url(#paperGrad)" stroke="#8b6508" stroke-width="1.5"/>
                    <rect x="0" y="0" width="22" height="60" rx="4" fill="url(#rollGrad)" stroke="#5c4000" stroke-width="2" filter="url(#innerShadow)"/>
                    <path d="M 15 4 L 22 4 L 22 56 L 15 56 Z" fill="rgba(0,0,0,0.15)"/> <rect x="178" y="0" width="22" height="60" rx="4" fill="url(#rollGrad)" stroke="#5c4000" stroke-width="2" filter="url(#innerShadow)"/>
                    <path d="M 178 4 L 185 4 L 185 56 L 178 56 Z" fill="rgba(0,0,0,0.15)"/> </svg>
            `;

            const jadeEmblemSvg = `
                <svg viewBox="0 0 100 100" width="100%" height="100%" style="filter: drop-shadow(2px 4px 6px rgba(0,0,0,0.6));">
                    <defs>
                        <radialGradient id="jadeGrad" cx="30%" cy="30%" r="70%">
                            <stop offset="0%" stop-color="#b2d8b2"/><stop offset="50%" stop-color="#5a8b5c"/><stop offset="100%" stop-color="#2a4d2c"/>
                        </radialGradient>
                        <linearGradient id="goldGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                            <stop offset="0%" stop-color="#fceda1"/><stop offset="50%" stop-color="#c19a3b"/><stop offset="100%" stop-color="#805b14"/>
                        </linearGradient>
                        <filter id="innerBevel"><feDropShadow dx="1" dy="1" stdDeviation="2" flood-color="#000" flood-opacity="0.6"/></filter>
                        <filter id="goldGlow"><feDropShadow dx="0" dy="0" stdDeviation="1" flood-color="#c19a3b" flood-opacity="0.5"/></filter>
                    </defs>
                    <circle cx="50" cy="50" r="48" fill="url(#jadeGrad)"/>
                    <circle cx="50" cy="50" r="48" fill="none" stroke="#2a4d2c" stroke-width="1"/>
                    
                    <circle cx="50" cy="50" r="34" fill="#e8f4e9" stroke="url(#goldGrad)" stroke-width="2.5" filter="url(#innerBevel)"/>
                    
                    <path d="M 50 16 A 17 17 0 0 1 50 50 A 17 17 0 0 0 50 84 A 34 34 0 0 1 50 16 Z" fill="#254627"/>
                    
                    <circle cx="50" cy="33" r="5" fill="#254627"/>
                    <circle cx="50" cy="67" r="5" fill="#e8f4e9"/>
                    
                    <circle cx="50" cy="50" r="34" fill="none" stroke="url(#goldGrad)" stroke-width="2" filter="url(#goldGlow)"/>
                </svg>
            `;

            statPopup.innerHTML = `
                <div class="iso-stat-close">X</div>
                <div class="iso-stat-header">
                    <div class="iso-stat-section active">Info</div>
                    <div class="iso-stat-section">Not decided</div>
                    <div class="iso-stat-section">Not decided</div>
                    <div class="iso-stat-section">Not decided</div>
                </div>
                <div class="iso-stat-body" id="stat-info-body">
                    
                    <div class="iso-jade-slab">
                        <div class="iso-jade-emblem">${jadeEmblemSvg}</div>
                        <div class="iso-jade-grid">
                            <div class="iso-jade-line"><span class="jade-label">Name:</span><span class="jade-val"></span></div>
                            <div class="iso-jade-line"><span class="jade-label">Gender:</span><span class="jade-val"></span></div>
                            <div class="iso-jade-line"><span class="jade-label">Alias:</span><span class="jade-val"></span></div>
                            <div class="iso-jade-line"><span class="jade-label">Lifespan:</span><span class="jade-val"></span></div>
                            
                            <div class="iso-jade-divider"></div>
                            
                            <div class="iso-jade-line"><span class="jade-label">Clan:</span><span class="jade-val"></span></div>
                            <div class="iso-jade-line"><span class="jade-label">Alignment:</span><span class="jade-val"></span></div>
                            <div class="iso-jade-line" style="grid-column: span 2;"><span class="jade-label">Affiliation:</span><span class="jade-val"></span></div>
                            
                            <div class="iso-jade-divider"></div>

                            <div class="iso-jade-line"><span class="jade-label">Reputation:</span><span class="jade-val"></span></div>
                            <div class="iso-jade-line"><span class="jade-label">Dao Vow:</span><span class="jade-val"></span></div>
                        </div>
                    </div>
                    
                    <div class="iso-stat-bio">
                        Biography:<br>
                        (Your character history goes here...)
                    </div>

                    <div class="iso-stat-buttons">
                        <div class="iso-scroll-btn">
                            ${scrollSvg}
                            <span class="iso-scroll-btn-text">Relation</span>
                        </div>
                        <div class="iso-scroll-btn">
                            ${scrollSvg}
                            <span class="iso-scroll-btn-text">Karma</span>
                        </div>
                    </div>
                </div>
            `;
            document.body.appendChild(statPopup);

            // --- TEMPORARY MULTI-EDITOR LOGIC ---
            const editor = document.createElement('div');
            editor.className = 'iso-editor-panel';
            editor.innerHTML = `
                <div class="editor-group">
                    <strong>JADE SLAB</strong>
                    <div class="editor-row"><label>X: <span id="val-slabX">4</span>vw</label><input type="range" id="slabX" min="0" max="100" value="4"></div>
                    <div class="editor-row"><label>Y: <span id="val-slabY">1</span>vw</label><input type="range" id="slabY" min="0" max="100" value="1"></div>
                    <div class="editor-row"><label>W: <span id="val-slabW">60</span>vw</label><input type="range" id="slabW" min="10" max="100" value="60"></div>
                </div>
                <div class="editor-group">
                    <strong>EMBLEM (SVG)</strong>
                    <div class="editor-row"><label>X: <span id="val-embX">-3.8</span>vw</label><input type="range" id="embX" min="-20" max="50" step="0.1" value="-3.8"></div>
                    <div class="editor-row"><label>Y: <span id="val-embY">50</span>%</label><input type="range" id="embY" min="0" max="100" value="50"></div>
                    <div class="editor-row"><label>Size: <span id="val-embS">7.5</span>vw</label><input type="range" id="embS" min="1" max="20" step="0.1" value="7.5"></div>
                </div>
                <div class="editor-group">
                    <strong>BUTTONS</strong>
                    <div class="editor-row"><label>X (Right): <span id="val-btnX">0</span>vw</label><input type="range" id="btnX" min="0" max="50" value="0"></div>
                    <div class="editor-row"><label>Y (Bottom): <span id="val-btnY">0</span>vw</label><input type="range" id="btnY" min="0" max="50" value="0"></div>
                    <div class="editor-row"><label>Btn W: <span id="val-btnW">11</span>vw</label><input type="range" id="btnW" min="5" max="30" value="11"></div>
                </div>
            `;
            document.body.appendChild(editor);

            const slab = statPopup.querySelector('.iso-jade-slab');
            const emblem = statPopup.querySelector('.iso-jade-emblem');
            const btnGroup = statPopup.querySelector('.iso-stat-buttons');
            const btns = statPopup.querySelectorAll('.iso-scroll-btn');

            const updateSlab = () => {
                slab.style.left = document.getElementById('slabX').value + 'vw';
                slab.style.top = document.getElementById('slabY').value + 'vw';
                slab.style.width = document.getElementById('slabW').value + 'vw';
                document.getElementById('val-slabX').textContent = document.getElementById('slabX').value;
                document.getElementById('val-slabY').textContent = document.getElementById('slabY').value;
                document.getElementById('val-slabW').textContent = document.getElementById('slabW').value;
            };

            const updateEmblem = () => {
                emblem.style.left = document.getElementById('embX').value + 'vw';
                emblem.style.top = document.getElementById('embY').value + '%';
                emblem.style.width = document.getElementById('embS').value + 'vw';
                emblem.style.height = document.getElementById('embS').value + 'vw';
                document.getElementById('val-embX').textContent = document.getElementById('embX').value;
                document.getElementById('val-embY').textContent = document.getElementById('embY').value;
                document.getElementById('val-embS').textContent = document.getElementById('embS').value;
            };

            const updateButtons = () => {
                btnGroup.style.right = document.getElementById('btnX').value + 'vw';
                btnGroup.style.bottom = document.getElementById('btnY').value + 'vw';
                btns.forEach(b => b.style.width = document.getElementById('btnW').value + 'vw');
                document.getElementById('val-btnX').textContent = document.getElementById('btnX').value;
                document.getElementById('val-btnY').textContent = document.getElementById('btnY').value;
                document.getElementById('val-btnW').textContent = document.getElementById('btnW').value;
            };

            editor.addEventListener('input', (e) => {
                if (e.target.id.startsWith('slab')) updateSlab();
                if (e.target.id.startsWith('emb')) updateEmblem();
                if (e.target.id.startsWith('btn')) updateButtons();
            });

            const closeStat = statPopup.querySelector('.iso-stat-close');
            closeStat.onclick = (e) => { e.stopPropagation(); statPopup.classList.remove('visible'); };
            ...
            staminaBar.innerHTML = `
                ...
                <div class="iso-ui-bars-column">
                    <div class="iso-nameplate" id="iso-health-clicker">
                        <div class="iso-health-fill" id="health-bar-fill"></div>
                        <div class="iso-health-text" id="iso-player-name">Player 100/100</div>
                    </div>

                    <div class="iso-ui-bar-wrapper">
                        <div class="iso-ui-bar-bg iso-torn-shape"></div>
                        <div class="iso-ui-bar-fill-area iso-torn-shape">
                            <div class="iso-ui-bar-progress stamina iso-torn-shape"></div>
                            <div class="iso-ui-text-layer">
                                <span class="iso-ui-label">STAMINA</span>
                                <div class="flex items-baseline">
                                    <span class="iso-ui-val" id="stamina-val-cur">100</span>
                    ...
                    <div class="iso-ui-bar-wrapper">
                        <div class="iso-ui-bar-bg iso-torn-shape"></div>
                        <div class="iso-ui-bar-fill-area iso-torn-shape">
                            <div class="iso-ui-bar-progress qi iso-torn-shape"></div>
                            <div class="iso-ui-text-layer">
                                <span class="iso-ui-label">QI POWER</span>
                                <div class="flex items-baseline">
                                    <span class="iso-ui-val" id="qi-val-cur">100</span>
                                    <span class="iso-ui-max">/ 100</span>
                                </div>
                            </div>
                        </div>
            ...
            const healthBtn = document.getElementById('iso-health-clicker');
            healthBtn.onclick = (e) => {
                e.stopPropagation();
            ...
            copyBtn.onclick = () => {
                ...
                // Fallback method that bypasses Permission Policy
        ...
        parseMapData(dataStr) {
            ...
            // Parse Player Name (Health bar update)
            const nameMatch = dataStr.match(/<Player>\s*Name\s*:\s*(.*)\s*<\/Player>/i);
            if (nameMatch) {
                const el = document.getElementById('iso-player-name');
                if (el) el.textContent = `${nameMatch[1].trim()} 100/100`;
            }
            ...
            while ((nMatch = npcRegex.exec(dataStr)) !== null) {
                ...
                while ((entry = entryRegex.exec(content)) !== null) {
                    ...
                    const type = getV(inner, /Type\s*:\s*(R|W)/i).trim();
                    const race = getV(inner, /Race\s*:\s*(.*)/i).trim();
                    const posMatch = inner.match(/Tile\s*=\s*(\d+)[.,](\d+)/i);

                    if (posMatch) {
                        const k = `${posMatch[1]},${posMatch[2]}`;
                        this.npcs[k] = { name, type, race };
        ...
        buildPathTo(targetTile) {
            ...
            while (currLx !== targetTile.lx) {
                currLx += Math.sign(targetTile.lx - currLx);
                const tile = Object.values(this.chunks).find(t =>
                    t.chunkId === this.player.chunkId && t.lx === currLx && t.ly === currLy
                );
                if (tile) path.push({ tile: tile, step: stepCount++ });
            }

            // Y direction increments
            while (currLy !== targetTile.ly) {
                currLy += Math.sign(targetTile.ly - currLy);
                const tile = Object.values(this.chunks).find(t =>
                    t.chunkId === this.player.chunkId && t.lx === currLx && t.ly === currLy
                );
                if (tile) path.push({ tile: tile, step: stepCount++ });
        ...
        handleTileClick(tile, isRightClick) {
            ...
            if (this.state === 'SHOW_RANGE') {
                ...
                if (move) {
                    if (move.isBlocked) {
                        this.triggerShake();
                        return; // Exit early to keep tiles visible
                    }

                    // Calculate Impedance-based cost from region data
                    let maxImpedance = 1; this.regions.forEach(reg => {                        // Improved coordinate range check to handle inverted ranges
        ...
        calculateManhattanMoves() {
            ...
            Object.values(this.chunks).forEach(tile => {
                ...
                const dist = Math.abs(tile.lx - pLx) + Math.abs(tile.ly - pLy);
                if (dist <= range) {
                    // Check if tile contains a Mountain or an NPC piece
                    const isMountain = tile.assetType === 'M';
                    const hasNPC = this.npcs && this.npcs[`${tile.lx},${tile.ly}`];
                    const isBlocked = isMountain || hasNPC;

                    this.validMoves.push({
                        key: `${tile.gx},${tile.gy}`,
                        tile: tile,
                        dist: dist,
                        isBlocked: !!isBlocked // Mark for rendering and logic
                    });
                }
            });
        }
        ...
        drawTile(tile, timestamp) {
            ...
            if (isHighlightState) {
                ...
                if (move) {
                    ...
                    if (timeElapsed > delay) {
                        ...
                        // Determine color: Blue for movement, Gold for sense, Red for blocked
                        let colorBase = (this.state === 'SENSE_ACTIVE') ? CONFIG.COLORS.HIGHLIGHT_GOLD : CONFIG.COLORS.HIGHLIGHT_BLUE;
                        if (move.isBlocked && this.state !== 'SENSE_ACTIVE') {
                            colorBase = CONFIG.COLORS.HIGHLIGHT_RED;
                        }
            ...
            // --- REGION BORDER (Attached to Tile) ---
            if (this.showRegions) {
                this.regions.forEach(reg => {
                    // Normalize coordinates (handle if x1 > x2)
                    const minX = Math.min(reg.x1, reg.x2);
                    const maxX = Math.max(reg.x1, reg.x2);
                    const minY = Math.min(reg.y1, reg.y2);
                    const maxY = Math.max(reg.y1, reg.y2);

                    if (tile.lx >= minX && tile.lx <= maxX && tile.ly >= minY && tile.ly <= maxY) {
                        // 1. Draw Region Fill
                        this.ctx.fillStyle = `rgba(76, 175, 80, 0.15)`;
                        this.ctx.fill();

                        // 2. Draw Borders on Edge Tiles
                        this.ctx.lineWidth = 3 * this.camera.zoom;
                        this.ctx.strokeStyle = '#4CAF50';
                        this.ctx.lineCap = 'round';
                        this.ctx.setLineDash([10, 5]); // Dashed Look

                        // Geometry Points (Relative to drawing origin)
                        // Top: x, y | Right: x + w/2, y + h/2 | Bottom: x, y + h | Left: x - w/2, y + h/2

                        this.ctx.beginPath();

                        // "Top-Left" Edge (Matches minX boundary)
                        if (tile.lx === minX) {
                            this.ctx.moveTo(x, y);
                            this.ctx.lineTo(x - w / 2, y + h / 2);
                        }

                        // "Bottom-Right" Edge (Matches maxX boundary)
                        if (tile.lx === maxX) {
                            this.ctx.moveTo(x + w / 2, y + h / 2);
                            this.ctx.lineTo(x, y + h);
                        }

                        // "Top-Right" Edge (Matches minY boundary)
                        if (tile.ly === minY) {
                            this.ctx.moveTo(x, y);
                            this.ctx.lineTo(x + w / 2, y + h / 2);
                        }

                        // "Bottom-Left" Edge (Matches maxY boundary)
                        if (tile.ly === maxY) {
                            this.ctx.moveTo(x - w / 2, y + h / 2);
                            this.ctx.lineTo(x, y + h);
                        }
        ...
        drawNPC(lx, ly, npcData) {
            ...
            const nImg = AssetManager.get(CONFIG.NPC_ASSETS[npcData.type]);
            const radius = w * 0.25;
            ...
            if (nImg && nImg.naturalWidth > 0) {
                this.ctx.drawImage(nImg, -radius, -radius, radius * 2, radius * 2);
            }
            this.ctx.restore();
        ...
        drawPlayer() {
            ...
            const modes = ['WalkUI.png', 'RunUI.png', 'SneakUI.png', 'HideUI.png'];
            const isHiding = modes[this.modeIdx || 0] === 'HideUI.png';
        ...
        drawRegions(timestamp) {
            ...
            this.regions.forEach(reg => {
                ...
                this.ctx.strokeStyle = '#4CAF50'; // Solid Green
                this.ctx.lineWidth = 4 * this.camera.zoom; // Thicker
        ...
        render(timestamp) {
            ...
            if (this.shakeIntensity > 0.1) {
                ...
                this.shakeIntensity *= 0.8; // Decay speed
            } else {
                this.shakeIntensity = 0;
            }
            ...
            // 1. Draw World (Includes Region Borders now)
            sortedTiles.forEach(tile => this.drawTile(tile, timestamp));

            // 2. Draw Entities (NPC Pieces followed by Player)
            Object.entries(this.npcs).forEach(([coords, data]) => {
                const [lx, ly] = coords.split(',').map(Number);
                this.drawNPC(lx, ly, data);
            });
            this.drawPlayer();
